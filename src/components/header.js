...
class Header extends React.Component {
  ...
  render () {
    ...
    const HubScroll = ({ children }) => {
      ...
      return (
        <Link to="/about" ref={revealRef} className="hidden md:block border-2 border-white uppercase text-white p-3 xl:p-4 font-medium text-lg force-hover-opacity">Learning Hub</Link>
      );
    };
    ...
    const MenuScroll = ({ children }) => {
      ...
      return (
        <button className="p-4 ml-3 force-hover-opacity" ref={revealRef}>
          <svg xmlns="http://www.w3.org/2000/svg" className="w-6 md:w-8" viewBox="0 0 32 20"><g data-name="Group 3" fill="#fff"><path data-name="Rectangle 3" d="M0 0h32v4H0z"/><path data-name="Rectangle 4" d="M0 16h32v4H0z"/></g></svg>
        </button>
      );
    };

    return (
      <header className="fixed top-0 left-0 right-0 z-30 bg-transparent pt-8 md:pt-12">
